...
define(function (require, exports, module) {
    ...
    var CarouselController = $.extend({
        ...
        dataBinding: function (data) {
            var _self = this
            items = '';
            doT.exec('carousel/list-item.html', function (templateFun) {
                items = templateFun(data);
                items = $(items);
                _self.bindItemEvent(items);
                _self.carouselArea.append(items);
            });
        },
        bindEvent: function () {
            var _self = this;
         ...
         * @param  {[删除按钮]} ele
         */
        deleteCarousel: function (ele) {
            ele = $(ele);
            if (!ele.hasClass('delete_item')) return;
            var res = confirm('确定要删除这个响应事件吗?');
            if (res) {
