...
define(function (require, exports, module) {
    ...
    var SkuController = $.extend({
        ...
        dataBinding: function (data) {
            ...
            doT.exec('marketsku/list-item.html', function (templateFun) {
            ...
            $('#btnAddButton').bind('click', function () {
                ...
                addsku.init({
                    ...
                    callBack: function (result) {
                        if (result) {
                            //重新加载页面
                            window.location.href = '/Product/MarketSkuIndex';
                        } else {
                            alert('添加库存失败');
                        }
                    }
        ...
        bindItemEvent: function (item) {
            ...
            item.find('button.edit_item').click(function (event) {
                _self.editProductSku(this);
                event.stopPropagation();
                return false;
            });
            item.find('button.confirm_item').click(function (event) {
                _self.confirmProductSku(this);
                event.stopPropagation();
                return false;
            });
        ...
        editProductSku: function (ele) {
            ele = $(ele);
            if (!ele.hasClass('edit_item')) return;
            var parent = ele.parent('td');
            parent.find("button.addOne_item").css("display", "inline"); 
            parent.find("button.delOne_item").css("display", "inline");
            parent.find("button.delete_item").css("display", "inline");
            parent.find("button.confirm_item").css("display", "inline");
            parent.find("button.edit_item").css("display", "none"); 
            //var input = ele.parent('td'),
            //    id = input.attr('data-item_id'),
            //    item = input.parent('tr');
        ...
        confirmProductSku: function (ele) {
            ele = $(ele);
            if (!ele.hasClass('confirm_item')) return;
            var parent = ele.parent('td');
            parent.find("button.addOne_item").css("display", "none");
            parent.find("button.delOne_item").css("display", "none");
            parent.find("button.delete_item").css("display", "none");
            parent.find("button.confirm_item").css("display", "none");
            parent.find("button.edit_item").css("display", "inline"); 
            //var input = ele.parent('td'),
            //    id = input.attr('data-item_id'),
            //    item = input.parent('tr');
        ...
        bindDistrictInfo: function (parentId) {
            ...
            data = $.param(data, true);
            Global.get("/Shop/GetMarketShopListByRegionId", data, function (msg) {
                $(msg.items).each(function (index, itemData) {
