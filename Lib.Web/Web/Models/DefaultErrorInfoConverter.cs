...
using LibMain.Runtime.Validation;
using LibMain.UI;
using System;
...
namespace Lib.Web.Models
{
    ...
    {
        private readonly ILibWebModuleConfiguration _configuration;
        ...
        public DefaultErrorInfoConverter(ILibWebModuleConfiguration configuration)
        {
        ...
        public ErrorInfo Convert(Exception exception)
        {
            ...
            if (exception is AggregateException && exception.InnerException != null)
            {
                ...
                if (aggException.InnerException is UserFriendlyException || aggException.InnerException is LibValidationException)
                {
            ...
            if (exception is LibValidationException)
            {
                return new ErrorInfo(LibWebLocalizedMessages.ValidationError)
                {
                    ValidationErrors = GetValidationErrorInfos(exception as LibValidationException)
                };
            }
            ...
            return new ErrorInfo(LibWebLocalizedMessages.InternalServerError);
        }
        ...
        private static ErrorInfo CreateDetailedErrorInfoFromException(Exception exception)
        {
            ...
            if (exception is LibValidationException)
            {
                errorInfo.ValidationErrors = GetValidationErrorInfos(exception as LibValidationException);
            }
        ...
        private static ValidationErrorInfo[] GetValidationErrorInfos(LibValidationException validationException)
        {
